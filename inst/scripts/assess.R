...
file="traintestSTAR.J.50.10.200.RData"
...
ALLN<-NULL
ALLn<-NULL
cat("NROW(X)=",NROW(trainD2C@origX), length(trainD2C@Y))
print(testDAG@NDAG)
r=1
ncores=2
nedges=20
  ...
  FF<-foreach (i=r:(r+ncores-1)) %do%{
   ## for (i in  r:(r+ncores-1) ){
    ...
    if (!any(is.nan(observedData))){
      ...
      Ahat.IAMB<-(amat(iamb(data.frame(observedData),alpha=0.01,max.sx=MX.SX,optimized=TRUE)))
     
      print("IAMB")
      Ahat.IIAMB<-(amat(inter.iamb(data.frame(observedData),alpha=0.01,,max.sx=MX.SX,
                                   optimized=TRUE)))
      print("IIAMB")
      Ahat.FIAMB<-(amat(fast.iamb(data.frame(observedData),alpha=0.01,,max.sx=MX.SX,optimized=TRUE)))
      print("FIAMB")
      Ahat.PC<-(amat(si.hiton.pc(data.frame(observedData),alpha=0.01,,max.sx=MX.SX,optimized=TRUE)))
      print("PC")
      ...
      for(jj in 1:NROW(subset.edges)){
        ...
        Yhat.D2C.1<-c(Yhat.D2C.1,as.numeric(pred.D2C$response)  -1)
